...
with open(file_path, 'r') as file:
    ...
    poll_config = json_data.get("poll", {})
    poll_enabled = bool(poll_config.get("enabled", False))
    poll_start = poll_config.get("start", "07:50")
    poll_end = poll_config.get("end", "18:00")
    poll_interval = int(poll_config.get("interval_minutes", 5))
    ...
    if not json_data['configLock']:
        ...
        print("配置已读取")
        if poll_enabled:
            print("当前签到模式为：轮询，%s-%s 每 %d 分钟检查一次"%(poll_start, poll_end, poll_interval))
        elif scheduletime=="":
...
def polling_loop():
    print("☆轮询已启动：%s-%s 每 %d 分钟检查一次☆"%(poll_start, poll_end, poll_interval))
    while True:
        now = datetime.now()
        delay = get_next_poll_delay(now, poll_start, poll_end, poll_interval)
        if delay is None:
            print("当前时间已超过%s，今日轮询结束。"%(poll_end))
            break
        if delay > 0 and now.strftime("%H:%M") < poll_start:
            print("当前时间为%s，等待到%s开始轮询。"%(now.strftime("%H:%M"), poll_start))
            time.sleep(delay)
            continue

        job()
        print("等待%d分钟后再次检查。"%poll_interval)
        time.sleep(delay)
