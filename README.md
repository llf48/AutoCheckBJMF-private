...
- 采集标签层级、属性/字段名、数据类型、函数调用名称、JSON 属性名，以及去除参数值的 URL 形状。隐藏字段、内联事件和脚本也会检查，但不保存参数原值、页面正文、Cookie、令牌、完整 HTML 或可执行脚本。
- 样本格式版本 2 为外部脚本增加 `source_hint`：对代码中允许的公共资源主机（班级魔方静态资源、微信 SDK、常见公共 CDN），保留经过校验的 `url_without_query`，用于人工追踪脚本来源。相对地址按实际页面地址及首个 `<base href>` 解析；不下载或执行脚本，不携带 Cookie，不读取浏览器后续请求。
- 查询参数一律只保留名称，删除全部值（包括 `a`、`v`），同时删除片段。未知主机、带用户名密码、异常端口、编码路径、疑似账号/凭据路径等只保留原有结构（`status=shape_only`）；域名允许列表及路径筛选见 `discovery_snapshot.py`。`public_asset_reference` 仅表示通过记录规则，不代表已访问或验证为公开资源。合并脚本可能依赖被删掉的参数，所以记录的地址不保证能直接下载；不能据此认定脚本内没有任务 ID。
